...
def get_memory_service():
    """Process-wide memory service using Vertex AI Memory Bank when configured."""
    ...
    agent_engine_id = os.environ.get(
        "GOOGLE_CLOUD_AGENT_ENGINE_ID", "1168349851767799808"
    )
    if agent_engine_id:
        from google.adk.memory import VertexAiMemoryBankService

        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT", "qwiklabs-gcp-03-7e288e21440f"),
            location=os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION")
