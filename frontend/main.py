...
def _load_default_resource() -> str:
    env_res = os.environ.get("AGENT_ENGINE_RESOURCE_NAME")
    if env_res:
        return env_res
    # Try reading deployment_metadata.json in parent or current directory
    for path in [Path("../deployment_metadata.json"), Path("deployment_metadata.json")]:
        if path.exists():
            try:
                ...
                pass
    raise RuntimeError("AGENT_ENGINE_RESOURCE_NAME env var not set and deployment_metadata.json not found")

RESOURCE = _load_default_resource()
